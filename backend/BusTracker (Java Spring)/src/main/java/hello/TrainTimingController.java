...
public class TrainTimingController {
    ...
    public int getTrainTimingAPI(String startID, String endID) {
        try {
            HttpResponse<JsonNode> jsonResponse = Unirest.get("https://maps.googleapis.com/maps/api/directions/json?origins=place_id:"+startID+"&destinations=place_id:"+endID+"&mode=transit&transit_mode=train&key=AIzaSyCR0lbKFMJUV4pTdj4a5KIgoy95tEOAxBA")
                    .asJson();
