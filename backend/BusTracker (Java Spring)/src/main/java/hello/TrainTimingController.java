...
public class TrainTimingController {
    ...
    public int getTrainTimingAPI(String startID, String endID) {
        try {
            HttpResponse<JsonNode> jsonResponse = Unirest.get("https://maps.googleapis.com/maps/api/directions/json?origin=place_id:"+startID+"&destination=place_id:"+endID+"&mode=transit&transit_mode=subway&key=AIzaSyCR0lbKFMJUV4pTdj4a5KIgoy95tEOAxBA")
                    .asJson();
